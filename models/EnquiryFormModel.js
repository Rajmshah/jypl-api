export default {
        ...
        async.parallel(
            {
                result: function(callback) {
                    EnquiryForm.find(filter)
                        .skip(skipVal)
                        .limit(pageLimit)
                        .exec(callback)
                },
                count: function(callback) {
                    EnquiryForm.countDocuments(filter).exec(callback)
                }
            },
            callback
        )
    ...
    createEnquiryForm(data, callback) {
        const enquiryForm = new EnquiryForm(data)
        enquiryForm.save(callback)
    },
    delete(data, callback) {
        var obj = {
            _id: data.id
        }
        EnquiryForm.deleteOne(obj, callback)
    },
    updateData: (param, data, callback) => {
        ...
        EnquiryForm.updateOne({ _id: param.id }, { $set: data2 }).exec(callback)
    },
